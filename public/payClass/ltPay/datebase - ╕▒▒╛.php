...
/*		//检测银行信息
	...
	public function runUpdate($array=array(),$isScan=false){
		...
		//更新用户充值信息
		$updateUserRecharge = $this->db_update($mysqli, 'ly_user_recharge', $updateUserRechargeArray, array('order_number'=>$array['order_number'],'uid'=>$array['uid'],'state'=>3,'money'=>$array['money']));
	...
	public function verifyDraw($array=array()){
		...
		if (!isset($array['price']) || !$array['price']) return '参数缺失';
		if (!isset($array['order_number']) || !$array['order_number']) return '参数缺失';
		// 连接数据库
		$mysqli = $this->db_connect();
		if (is_null($mysqli)) return '数据库连接失败';
		//检测订单
		$countDraw = $this->db_select($mysqli, 'ly_user_withdrawals', array('order_number'=>$array['order_number'],'price'=>$array['price'],'state'=>4), 'count(*)');
		if(!$countDraw['count(*)']){
		    mysqli_close($mysqli);
		    return '订单不存在';
		}
	 ...
	 * @return [type] [description]
	 */
	public function withDrawRun($array=array(),$msg=false){
		if(!$array) return false;
		// 连接数据库
		$mysqli = $this->db_connect();
		...
		if($msg){
			// 更新用户提现信息，出款失败
			$this->db_update($mysqli, 'ly_user_withdrawals', ['state'=>5], ['order_number'=>$array['order_number'],'price'=>$array['price'],'state'=>4]);
			mysqli_close($mysqli);
			return false;
		}

		$countDraw = $this->db_select($mysqli, 'ly_user_withdrawals', array('order_number'=>$array['order_number'],'price'=>$array['price'],'state'=>4),'`uid`');
		if(!$countDraw){
			mysqli_close($mysqli);
			return false;
		}
		$array['uid'] = $countDraw['uid'];
		// 更新用户提现信息
		$updateWithDraw = $this->db_update($mysqli, 'ly_user_withdrawals', ['state'=>6], ['uid'=>$array['uid'],'order_number'=>$array['order_number'],'price'=>$array['price'],'state'=>4]);
		if(!$updateWithDraw){
			mysqli_close($mysqli);
			return false;
		}
		// 更新流水
		$updateWithDraw1 = $this->db_update($mysqli, 'ly_trade_details', ['state'=>1], ['uid'=>$array['uid'],'order_number'=>$array['order_number'],'state'=>3]);
		if(!$updateWithDraw1){
			$this->db_update($mysqli, 'ly_user_withdrawals', ['state'=>4], ['uid'=>$array['uid'],'order_number'=>$array['order_number'],'price'=>$array['price'],'state'=>6]);
			mysqli_close($mysqli);
			return false;
		}
		// 更新报表
		...
		if(!$countReport['count(*)']){
			$userInfo = $this->db_select($mysqli, 'ly_users', array('id'=>$array['uid'],'recharge_state'=>1), '`username`,`user_type`');
	        $updateDaily = $this->db_insert($mysqli, 'ly_user_daily', array('uid'=>$array['uid'],'username'=>$userInfo['username'],'user_type'=>$userInfo['user_type'],'date'=>$today,'withdrawals'=>$array['price']));
	    }else{
