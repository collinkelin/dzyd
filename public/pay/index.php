  ...
  <div class="PaymentTips">
    <div class="con">
      ...
      <div class="time">
        ...
        <a class="submit" id="SubmitBtn" href="javascript:;" style="<?php if($dataArray['payWay']=='WechatPay'){?>background-color: #07c160<?php }?>">确认</a>
    </div>
...
$('#SubmitBtn').click(function(event) {
  ...
  var autojump = $('input[name="autoJump"]').val()||0;
  $('#Loading').show();
  $.ajax({
    url: form.action,
    type: "post",
    ...
    success: function(data) {
      ...
      if(data.code==0){
        ...
        if(autojump){
          location.replace('payauto.php');
        }else{
          location.replace('paycode.php');
        }
      }else{
        $('#Loading').hide();
        $('.PaymentTips .con').html('<h2>重要提示</h2><div class="notice" style="text-align:center;"><p>'+data.message+'</p><p>请关闭此页面，重新下单</p></div>');
        sessionStorage.setItem('FailOrder',1);
      }
    },
    error: function(e) {
...
$(document).ready(function(){
  ...
  $.ajax({
    ...
    error: function(e) {
      $('#Loading').hide();
      $('.PaymentTips .con').html('<h2>重要提示</h2><div class="notice" style="text-align:center;"><p>提交失败</p><p>请关闭此页面，重新下单</p></div>');
    }
  });
})
</script>
