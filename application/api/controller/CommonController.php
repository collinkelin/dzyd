...
class CommonController extends BaseController{
	...
	//	$UserVipData	= model('UserVip')->where(array(['etime','>=',strtotime(date("Y-m-d",time()))],['state','=',1]))->order('etime','DESC')->limit(10)->select()->toArray();
		...
		if($UserVipData){
			foreach ($UserVipData as $key2 => $value2) {
                //$userviplist[$key2]['number'] = rand(1,100);
	...
	//	}
		...
		switch($lang){
			case 'en':
				$authenticationdata = ['Mobile phone authentication','Real name authentication','Identity authentication'];
			break;
			case 'cn':
				$authenticationdata	=['手机认证','实名认证','身份认证'];
			break;
			case 'ft':
				$authenticationdata	=['手機認證','實名認證','身份認證'];
			break;
			case 'vi':
			...
			break;
			case 'yd':
				$authenticationdata	=['मोबाइल फोन प्रमाणीकरण', 'वास्तविक नाम प्रमाणीकरण', 'पहिचान प्रमाणीकरण'];
			break;
		}
		...
		$data['info']['link']		=	['http://m.m1mz0g.com','http://m.m1mz0g.com','http://m.m1mz0g.com'];
