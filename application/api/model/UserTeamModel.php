...
class UserTeamModel extends UT{
	...
	public function teamReport(){
		...
		//查询符合条件的数据
		$userList = model('Users')->field('id,sid,username,state')->where('sid', $sid)->select()->toArray();
		//用户团队数据计算
		$data['teamData'] = model('manage/UserDaily')->teamStatistic($userList,$startDate,$endDate,$sid);
		...
		//一二三级统计
        //充值金额，充值人数，充值返佣
        $data['team1'] = ['teamRechargeCount'=>0,'teamRechargeNumber'=>0,'teamSpreadSum'=>0,];//一级代理
        $data['team2'] = ['teamRechargeCount'=>0,'teamRechargeNumber'=>0,'teamSpreadSum'=>0,];//二级代理
        $data['team3'] = ['teamRechargeCount'=>0,'teamRechargeNumber'=>0,'teamSpreadSum'=>0,];//三级代理
        $merchant1_users = [];
        $merchant2_users = [];
        $merchant3_users = [];
        $merchant1_users = self::getUids($userList);

        if(!empty($merchant1_users)){
            $merchant2_users_data = model('Users')->field('id,sid,username,state')->where('sid','in', $merchant1_users)->select()->toArray();
            $merchant2_users = self::getUids($merchant2_users_data);
        }
        if(!empty($merchant2_users)){
            $merchant3_users_data = model('Users')->field('id,sid,username,state')->where('sid','in', $merchant2_users)->select()->toArray();
            $merchant3_users = self::getUids($merchant3_users_data);
        }
        //总充值
        $data['team1']['teamRechargeCount'] = round($this->alias('ut')->join('user_recharge','ut.team=user_recharge.uid')->where('ut.uid','in',$merchant1_users)->where('user_recharge.state','=',1)->sum('money'),2);
        $data['team2']['teamRechargeCount'] = round($this->alias('ut')->join('user_recharge','ut.team=user_recharge.uid')->where('ut.uid','in',$merchant2_users)->where('user_recharge.state','=',1)->sum('money'),2);
        $data['team3']['teamRechargeCount'] = round($this->alias('ut')->join('user_recharge','ut.team=user_recharge.uid')->where('ut.uid','in',$merchant3_users)->where('user_recharge.state','=',1)->sum('money'),2);
        //充值人数
        $data['team1']['teamRechargeNumber'] = count($merchant1_users);
	...
	public static function getUids($userlist=[])
    {
        $uids = [];
        if(!empty($userlist)){
