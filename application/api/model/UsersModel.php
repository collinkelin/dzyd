...
class UsersModel extends Model{
	...
	public function register(){
        ...
        //更新新的邀请码
        $up_idcode = str_pad($insert_id,5,"0",STR_PAD_LEFT);
        model('Users')->where('id',$insert_id)->update(['idcode'=>$up_idcode,'uid'=>$up_idcode]);
		// 汇总表
...
//                echo model('UserDaily')->getLastSql();
	...
	public function getuserinfo(){
		...
		//ID设置五位数
        $data['info']['idcode'] = str_pad($data['info']['userid'],5,"0",STR_PAD_LEFT);
        $data['info']['sid'] = str_pad($data['info']['sid'],5,"0",STR_PAD_LEFT);
		return $data;
	}
	...
	public function setuserinfo(){
		...
		if(isset($param['is_housekeeper']) && $param['is_housekeeper'] && $userinfo['is_housekeeper']==0){
			if($userinfo['vip_level']<5){
				if($lang=='cn'){
					return ['code' => 0, 'code_dec' => '最低必须是奥斯卡才可以开通此服务'];
				}else{
					return ['code' => 0, 'code_dec' => 'At least Oscar is required to open this service !'];
				}
			}
			$amount	=	99;
			// 检测用户的余额
			...
			$param['housekeeper_time']	=	strtotime(date("Y-m-d",time())) + 30 * 24 * 3600;
		}
	...
	**/
	public function signin(){
		...
		if($in){
			...
			else return ['code' => 0, 'code_dec' => 'Signed in failed!'];
		}
